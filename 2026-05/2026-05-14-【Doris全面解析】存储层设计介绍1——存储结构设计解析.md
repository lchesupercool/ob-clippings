---
title: "【Doris全面解析】存储层设计介绍1——存储结构设计解析"
source: "https://mp.weixin.qq.com/s/aJ3FwDI6KprYYUwXzhl_-A"
author: "ApacheDoris"
published: ""
saved: "2026-05-14T21:22:33+08:00"
type: clipping
platform: wechat
---
...
来源：[原文](https://mp.weixin.qq.com/s/aJ3FwDI6KprYYUwXzhl_-A)
作者：ApacheDoris
发布时间：未知
保存时间：2026-05-14T21:22:33+08:00

> 本文介绍了Segment V2版本的存储层结构，包括有序存储、稀疏索引、前缀索引、位图索引、BloomFilter等丰富功能，可以应对各种复杂的场景提供极速的查询能力。

本文转自开源中国ZhangYu0123博客
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-01.png)

## **3.2 Segment v2文件结构**

Segment整体的文件格式分为数据区域，索引区域和footer三个部分，如下图所示：

![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-02.png)
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-03.png)
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-04.png)
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-05.png)
...
TINYINT/SMALLINT/INT/BIGINT/LARGEINTBIT_SHUFFLEFLOAT/DOUBLE/DECIMALBIT_SHUFFLECHAR/VARCHARDICTBOOLRLEDATE/DATETIMEBIT_SHUFFLEHLL/OBJECTPLAIN
默认采用LZ4F格式对数据进行压缩。
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-06.png)
...
ColumnNameTypeuser_idBIGINTageINTmessageVARCHAR(100)max_dwell_timeDATETIMEmin_dwell_timeDATATIME
（2）以下表结构的前缀索引为 user_name(20 Bytes)。即使没有达到 36 个字节，因为遇到 VARCHAR，所以直接截断，不再往后继续。
ColumnTypeuser_nameVARCHAR(20)ageINTmessageVARCHAR(100)max_dwell_timeDATETIMEmin_dwell_timeDATETIME
当我们的查询条件，是前缀索引的前缀时，可以极大的加快查询速度。比如在第一个例子中，我们执行如下查询：
SELECT*FROMtableWHEREuser_id=1829239andage=20；
该查询的效率会远高于如下查询：
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-07.png)
...
SELECT*FROMtableWHEREage >20andage <1000
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-08.png)
...
案例：table的schema如下：
ColumnNameTypeuser_idBIGINTageINTnameVARCHAR(20)cityVARCHAR(200)createtimeDATETIME
这里的查询sql如下：
SELECT*FROMtableWHEREname='张三'
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-09.png)
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-10.png)
...
ColumnNameTypeuser_idBIGINTageINTnameVARCHAR(20)cityVARCHAR(200)createtimeDATETIME
这里的查询sql如下：
SELECT*FROMtableWHEREcityin("北京","上海")
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-11.png)
...
![image](../../assets/2026-05-14-【Doris全面解析】存储层设计介绍1——存储结构设计解析/image-12.jpg)
