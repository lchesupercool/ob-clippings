...
- Attach documents, spreadsheets, presentations, or PDF files when you want
  ChatGPT to summarize, compare, transform, or [create files for review](/codex/artifacts-viewer).
- Add a screenshot, diagram, or other [image input](/codex/image-inputs) when the
  task depends on visual context. Point out the area that matters instead of
  relying on the image alone.
- Ask ChatGPT to use [web search](/codex/web-search) when the answer depends on
  current information, and ask for sources when you need to check the result.
- Use a [project](/codex/projects) when related chats or tasks should share files,
...
Find, install, and use plugins in ChatGPT.](/codex/plugins)
...
Set a default personality, custom instructions, and other app preferences.](/codex/app/settings#personalization)
...
[**Settings > General > Follow-up behavior**](/codex/app/settings#general).
...
[interactive shortcuts](/codex/developer-commands?surface=cli#cli-interactive-shortcuts)
for details.
...
![Voice dictation indicator in the composer with a transcribed prompt](../../assets/chatgpt-learn-prompting/voice-dictation-light.webp)
...
reliable, [schedule work from that task](/codex/automations#schedule-work-from-a-task).
...
investigate and propose an approach before editing. When [Goal mode](/codex/long-running-work)
is available, use `/goal` after the plan to set a persistent goal. See the [app slash
commands](/codex/reference/slash-commands)
...
Codex runs local commands inside a [sandbox](/codex/sandboxing)
...
1. If you haven’t already done so, set up a [Codex cloud environment](/codex/environments/cloud-environment).
...
about [cloud internet access](/codex/cloud/internet-access).
...
Before you can use this, enable Codex **Code review** on your repository. See [Code review](/codex/third-party/github).
