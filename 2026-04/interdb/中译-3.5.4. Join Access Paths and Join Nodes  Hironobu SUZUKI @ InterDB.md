# 3.5.4. Join 的 Access Path 与 Join 节点

## 3.5.4.1. Join Access Paths

`JoinPath` 结构体是 nested loop join 的 access path。其他 join 类型的 access path，例如 `MergePath` 和 `HashPath`，都以其为基础。

下面列出所有 join access path，不再附加解释。
...
![](https://www.interdb.jp/pg/pgsql03/05/fig-3-33.png)

图 3.33. Join access paths。

## 3.5.4.2. Join 节点

本小节展示三种 join 节点：`NestedLoopNode`、`MergeJoinNode` 和 `HashJoinNode`，不再附加解释。它们都以 `JoinNode` 为基础。
