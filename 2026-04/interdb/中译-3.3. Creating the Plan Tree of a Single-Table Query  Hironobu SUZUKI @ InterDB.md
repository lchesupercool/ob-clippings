# 3.3. 创建单表查询的 plan tree

由于 planner 的处理过程非常复杂，本节先描述最简单的情形，即如何为单表查询创建 plan tree。更复杂的处理——也就是如何为多表查询创建 plan tree——将在 `Section 3.6` 中描述。

PostgreSQL 中的 planner 执行以下三个步骤：

1. 执行预处理（preprocessing）。
2. 通过估算所有可能的 access path 的 cost，得到最便宜（cheapest）的 access path。
3. 基于最便宜的 path 创建 plan tree。

access path 是估算 cost 的处理单元。例如，sequential scan、index scan、sort 以及各种 join 操作都有与之对应的 path。access path 仅在 planner 内部用于创建 plan tree。

access path 最基础的数据结构是定义在 `pathnodes.h` 中的 `Path` 结构体，它对应 sequential scan。其他所有 access path 都以它为基础。具体细节将在下文的说明中描述。
...
为了处理上述步骤，planner 在内部创建一个 `PlannerInfo` 结构体，用于保存 query tree、query 中涉及的 relation 相关信息、access path 等。
...
本节将通过具体示例，描述如何从 query tree 创建 plan tree。
...
在创建 plan tree 之前，planner 会对存储在 PlannerInfo 结构体中的 query tree 做一些预处理。

尽管预处理涉及很多步骤，本小节只讨论针对单表查询的主要预处理。其他预处理操作在 `Section 3.6` 中描述。

预处理步骤包括：

1. 简化 target list、limit 子句等。
	例如，定义在 `clauses.c` 中的 eval\_const\_expressions() 函数会把 '2 + 2' 重写为 '4'。
2. 规范化布尔表达式。
	例如，'NOT (NOT a)' 被重写为 'a'。
3. 将 AND/OR 表达式扁平化。
	在 SQL 标准中，AND 和 OR 是二元运算符，但在 PostgreSQL 内部，它们是 n 元运算符，且 planner 始终假设所有嵌套的 AND 和 OR 表达式都应被扁平化。
	下面展示一个具体示例。考虑一个布尔表达式 '(id = 1) OR (id = 2) OR (id = 3)'。Figure 3.9(a) 展示了使用二元运算符时 query tree 的一部分。planner 通过使用三元运算符将该树扁平化以简化它。见 Figure 3.9(b)。
![](https://www.interdb.jp/pg/pgsql03/fig-3-09.png)

图 3.9. 扁平化 AND/OR 表达式的示例。

## 3.3.2. 获取最便宜的 access path

为了得到最便宜的 access path，planner 会估算所有可能的 access path 的 cost，并选出其中最便宜的一条。更具体地，planner 会执行以下操作：

1. 创建一个 `RelOptInfo` 结构体，用于存储 access path 及其对应的 cost。
	RelOptInfo 结构体由 make\_one\_rel() 函数创建，并存储在 PlannerInfo 结构体的 *simple\_rel\_array* 中。见 Figure 3.10。在初始状态下，RelOptInfo 持有 *baserestrictinfo*，以及当目标表存在相关 index 时的 *indexlist*。baserestrictinfo 存储 query 的 WHERE 子句，indexlist 存储目标表相关的 index。
...
1. 估算所有可能的 access path 的 cost，并把这些 access path 加入 RelOptInfo 结构体。
	此处理的详细步骤如下：
	1. 创建一条 path，估算 sequential scan 的 cost，并将估算出的 cost 写入该 path。然后把这条 path 加入 RelOptInfo 结构体的 pathlist。
		2. 如果目标表上存在相关的 index，则创建 index access path，估算所有 index scan 的 cost，并将估算出的 cost 写入 path。然后把这些 index path 加入 pathlist。
		3. 如果可以执行
		`bitmap scan`，则创建 bitmap scan path，估算所有 bitmap scan 的 cost，并将估算出的 cost 写入 path。然后把这些 bitmap scan path 加入 pathlist。
2. 从 RelOptInfo 结构体的 pathlist 中取出最便宜的 access path。
3. 如有必要，估算 LIMIT、ORDER BY 和 ARREGISFDD 的 cost。

为了让读者清楚理解 planner 的工作方式，下面给出两个具体示例。

### 3.3.2.1. 示例 1

首先，我们来看一个没有 index 的简单单表查询；这个 query 同时包含 WHERE 和 ORDER BY 子句。
...
Figures 3.10 和 3.11 描绘了 planner 在此例中的处理过程。

![](https://www.interdb.jp/pg/pgsql03/fig-3-10.png)

图 3.10. 示例 1 中如何获得最便宜的 path

1. 创建一个 RelOptInfo 结构体，并把它存入 PlannerInfo 的 simple\_rel\_array。
2. 将 WHERE 子句加入 RelOptInfo 的 baserestrictinfo。
	WHERE 子句 ' $id \lt 300$ ' 由定义在 `initsplan.c` 中的 distribute\_restrictinfo\_to\_rels() 函数加入 baserestrictinfo。此外，由于目标表没有相关的 index，RelOptInfo 的 indexlist 为 NULL。
3. 由定义在
	`planner.c` 中的 standard\_qp\_callback() 函数将排序用的 pathkey 添加到 PlannerInfo 的 sort\_pathkeys 中。
	*Pathkey* 是表示 path 排序次序的数据结构。在这个例子中，由于该 query 包含 ORDER BY 子句，其列为 'data'，因此列 'data' 作为一个 pathkey 被加入 sort\_pathkeys。
4. 创建一个 path 结构体，使用 cost\_seqscan 函数估算 sequential scan 的 cost，并将估算出的 cost 写入该 path。然后通过定义在
	`pathnode.c` 中的 add\_path() 函数把该 path 加入 RelOptInfo。
	如前所述，`Path` 结构体同时包含 start-up cost 和 total cost，这些都由 cost\_seqscan 等函数估算。

在这个示例中，由于目标表没有 index，planner 只估算 sequential scan 的 cost。因此，最便宜的 access path 自动确定。

![](https://www.interdb.jp/pg/pgsql03/fig-3-11.png)

图 3.11. 示例 1 中如何获得最便宜的 path。（续自 Figure 3.10）

1. 创建一个新的 RelOptInfo 结构体来处理 ORDER BY 过程。
	注意，这个新的 RelOptInfo 不持有 baserestrictinfo，也就是不含 WHERE 子句的信息。
2. 创建一条 sort path 并把它加入这个新的 RelOptInfo；然后将 sequential scan path 链接为 sort path 的 subpath。
	`SortPath` 结构体由两个 path 结构体组成：path 和 subpath；path 存储关于 sort 操作自身的信息，subpath 则存储最便宜的 path。
	请注意，sequential scan path 的 'parent' 项持有到旧 RelOptInfo 的链接，而旧 RelOptInfo 在其 baserestrictinfo 中存储了 WHERE 子句。因此在下一阶段——也就是创建 plan tree 时——尽管新的 RelOptInfo 没有 baserestrictinfo，planner 依然可以创建一个包含 WHERE 子句作为 'Filter' 的 sequential scan 节点。
...
基于此处得到的最便宜 access path，会生成一棵 plan tree。详情在 `Section 3.3.3` 中描述。

### 3.3.2.2. 示例 2

接下来，我们再看另一个带两个 index 的单表查询；这个 query 包含一个 WHERE 子句。
...
Figures 3.12 至 3.14 描绘了 planner 在此例中的处理过程。

![](https://www.interdb.jp/pg/pgsql03/fig-3-12.png)

图 3.12. 示例 2 中如何获得最便宜的 path。

1. 创建一个 RelOptInfo 结构体。
2. 将 WHERE 子句加入 baserestrictinfo，并将目标表的 index 加入 indexlist。
	在此示例中，WHERE 子句 ' id<240 $id<240$ $\text{id} \lt 240$ ' 被加入 baserestrictinfo，两个 index——*tbl\_2\_pkey* 和 *tbl\_2\_data\_idx*——被加入 RelOptInfo 的 indexlist。
3. 创建一条 path，估算 sequential scan 的 cost，并将该 path 加入 RelOptInfo 的 pathlist。
![](https://www.interdb.jp/pg/pgsql03/fig-3-13.png)

图 3.13. 示例 2 中如何获得最便宜的 path。（续自 Figure 3.12）

1. 创建一个 `IndexPath`，估算 index scan 的 cost，并使用 add\_path() 函数将该 IndexPath 加入 RelOptInfo 的 pathlist。
	在此示例中，由于存在两个 index——*tbl\_2\_pkey* 和 *tbl\_2\_data\_idx*——它们会按顺序被处理。*tbl\_2\_pkey* 被首先处理。
	为 *tbl\_2\_pkey* 创建一个 IndexPath，并同时估算 start-up cost 和 total cost。在此示例中，*tbl\_2\_pkey* 是列 'id' 的相关 index，而 WHERE 子句包含列 'id'；因此 WHERE 子句被存储在 IndexPath 的 *indexclauses* 中。
	注意，在向 pathlist 添加 access path 时，add\_path() 函数按 total cost 的排序顺序添加 path。在本例中，该 index scan 的 total cost 小于 sequential scan 的 total cost；因此这条 index path 被插入在 sequential scan path 之前。
...
1. 创建另一个 IndexPath，估算其他 index scan 的 cost，并把这条 index path 加入 RelOptInfo 的 pathlist。
	接着，为 *tbl\_2\_data\_idx* 创建一个 IndexPath，估算其 cost，并将该 IndexPath 加入 pathlist。在本例中，没有与 *tbl\_2\_data\_idx* index 相关的 WHERE 子句；因此 index clauses 为 NULL。

注

add\_path() 函数并非总是会把 path 加入。由于此操作较为复杂，相关细节在此从略。如需详情，请参阅 add\_path() 函数的注释。

![](https://www.interdb.jp/pg/pgsql03/fig-3-14.png)

图 3.14. 示例 2 中如何获得最便宜的 path。（续自 Figure 3.13）

1. 创建一个新的 RelOptInfo 结构体。
2. 把最便宜的 path 加入新 RelOptInfo 的 pathlist。
	在此示例中，最便宜的 path 是使用 index *tbl\_2\_pkey* 的 index path；因此这条 path 被加入新 RelOptInfo 的 pathlist。

## 3.3.3. 创建 Plan Tree

在最后阶段，planner 基于最便宜的 path 生成 plan tree。

plan tree 的根是定义在 `plannodes.h` 中的 `PlannedStmt` 结构体。它包含十九个字段，这里只列出其中四个有代表性的字段：

- **commandType** 存储操作的类型，例如 SELECT、UPDATE 或 INSERT。
- **rtable** 存储 rangeTable 条目。
- **relationOids** 存储此 query 涉及的相关表的 oid。
- **plantree** 存储由 plan node 组成的 plan tree，其中每个 node 对应一个具体的操作，例如 sequential scan、sort 和 index scan。
...
如上所述，plan tree 由各种 plan node 组成。`PlanNode` 结构体是基础 node，其他所有 node 都包含它。例如，用于 sequential scan 的 `SeqScanNode` 由一个 PlanNode 和一个整型变量 'scanrelid' 组成。PlanNode 包含十四个字段，下面列出其中七个有代表性的字段。

- **start-up cost** 和 **total\_cost** 是该 node 对应操作的估算 cost。
- **rows** 是要扫描的行数，由 planner 估算。
- **targetlist** 存储 query tree 中所含的 target list 项。
- **qual** 是一个 list，用于存储 qual 条件。
- **lefttree** 和 **righttree** 是用于添加子 node 的 node。
...
下文将描述两个 plan tree，它们是基于上一小节示例中展示的最便宜 path 生成的。

### 3.3.3.1. 示例 1

第一个例子是 `Section 3.3.2.1` 示例中生成的 plan tree。Figure 3.11 所示的最便宜 path 是由一条 sort path 和一条 sequential scan path 组成的树。根 path 是 sort path，子 path 是 sequential scan path。

尽管省略了详细说明，但不难理解 plan tree 几乎可以直接从最便宜的 path 生成。

在此示例中，一个 `SortNode` 被加入 PlannedStmt 结构体的 plantree，而一个 SeqScanNode 被加入 SortNode 的 lefttree。见 Figure 3.15(a)。
...
在 SortNode 中，lefttree 指向 SeqScanNode。

在 SeqScanNode 中，qual 持有 WHERE 子句 ' id<300 $id<300$ $\text{id} \lt 300$ '。

### 3.3.3.2. 示例 2

第二个例子是 `Section 3.3.2.2` 示例中生成的 plan tree。Figure 3.14 所示的最便宜 path 是 index scan path，因此该 plan tree 仅由一个 `IndexScanNode` 结构体组成。见 Figure 3.15(b)。
...
在此示例中，WHERE 子句 ' id<240 $id<240$ $\text{id} \lt 240$ ' 是一个 access predicate，因此它被存储在 IndexScanNode 的 indexqual 中。
